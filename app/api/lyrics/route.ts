...
// Fetches lyrics from the open lrclib.net API (free, no key required)
export async function GET(req: NextRequest) {
  ...
  try {
    const linkedTrack = findLinkedTrack({ trackId, videoId, title, artist })
    const storedLyrics = getStoredTrackLyrics(linkedTrack)
    if (storedLyrics) {
      return NextResponse.json({ ...storedLyrics, cached: true })
    }
...
function toLyricsResponse(data: any): LyricsResponse {
  const hasLyrics = Boolean(data?.syncedLyrics || data?.plainLyrics)

  return {
    plainLyrics: hasLyrics ? data.plainLyrics ?? null : null,
    syncedLyrics: hasLyrics ? data.syncedLyrics ?? null : null,
    source: hasLyrics ? "lrclib" : null,
  }
}
