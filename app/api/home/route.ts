...
export async function GET() {
  try {
    ...
    const recentTracks = (queries.getRecentTracks.all() as Track[]).slice(0, 20).map(toLocalTrack)
    const mostPlayedTracks = (queries.getMostPlayedTracks.all() as Track[]).slice(0, 20).map(toLocalTrack)
